...
class ROMFileUpload extends React.Component {
  state = {
    uploadedFileContents: new Array(1024),
  ...
  uploadFile = async event => {
    ...
    try {
      const fileContents = await ROMFileUpload.readUploadedFileAsText(latestUploadedFile);
      let sum = 0;
      ...
      for(let i=0,j=0,k=0;i<fileContents.length;i++){
        ...
        if(fileContents[i]=='0' || fileContents[i] == '1'){
          sum = sum + fileContents[i]*Math.pow(2,14-j);
          j++;
          if(j == 15){
